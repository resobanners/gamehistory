...
export default function GameTracker() {
  ...
  const handleShareImage = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(rosterRef.current, { backgroundColor: '#0e0e12', useCORS: true, scale: 2, windowWidth: 1400 });
  ...
  // Helper to chunk games into rows for the poster
  const getRows = (games) => {
    const rows = [];
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .share-btn { background: #c9b8ff; color: #000; border: none; padding: 12px 24px; border-radius: 12px; cursor: pointer; font-weight: 700; }

        /* UI GRID (NORMAL VIEW) */
        .grid-months { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; }
        .month-col { background: #16141f; border: 1px solid #2a2835; border-radius: 20px; padding: 15px; display: flex; flex-direction: column; min-height: 300px; }
        .month-label { font-family: 'Bebas Neue', sans-serif; font-size: 1.5rem; color: #c9b8ff; text-align: center; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid #2a2835; }
        
        .ui-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
        .ui-card { position: relative; aspect-ratio: 3/4; border-radius: 10px; overflow: hidden; background: #000; }
        .ui-card img { width: 100%; height: 100%; object-fit: cover; }
        ...
        .ui-add { aspect-ratio: 3/4; border: 2px dashed #2a2835; border-radius: 10px; color: #2a2835; font-size: 2rem; cursor: pointer; display: flex; align-items: center; justify-content: center; background: none; }

        /* 📸 POSTER SYSTEM (AUTO-SCALING SLOTS) */
        .is-exporting .grid-months { grid-template-columns: repeat(4, 1fr) !important; width: 1400px; padding: 40px; gap: 25px; }
        .is-exporting .month-col { height: 650px !important; padding: 20px; }
        .is-exporting .poster-container { display: flex; flex-direction: column; height: 100%; width: 100%; }
        ...
        /* MODAL */
        .modal-backdrop { position: fixed; inset: 0; background: rgba(0,0,0,0.9); z-index: 100; display: flex; align-items: center; justify-content: center; backdrop-filter: blur(10px); }
        ...
        .search-card { cursor: pointer; border-radius: 8px; overflow: hidden; }
        .search-card img { width: 100%; aspect-ratio: 3/4; object-fit: cover; }
      `}</style>
      ...
      <div className={`page ${isExporting ? 'is-exporting' : ''}`}>
        <header className="header" data-html2canvas-ignore="true">
          ...
          <button className="share-btn" onClick={handleShareImage}>📸 Poster Olarak Kaydet</button>
        </header>
          ...
          {MONTHS.map((month, mi) => {
            ...
            return (
              <div key={mi} className="month-col">
                ...
                {/* 1. POSTER VIEW (Only active during export) */}
                {isExporting && (
                  <div className="poster-container">
                    {getRows(games).map((row, ri) => (
